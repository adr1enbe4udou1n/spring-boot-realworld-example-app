...
public class ArticleUpdateTest extends RealworldApplicationTests {
  ...
  public void cannot_update_non_existent_article() {
    actingAsJohnUser()
        .contentType(ContentType.JSON)
        .body(new UpdateArticleRequest(new UpdateArticle()))
        .when()
        .put(baseUrl + "/api/articles/test-title")
        .then()
        .statusCode(401);
  ...
  public void can_update_own_article() {
    ...
    actingAs(user)
        .contentType(ContentType.JSON)
        .body(
            new UpdateArticleRequest(new UpdateArticle("New Title", "New Description", "New Body")))
        .when()
        .put(baseUrl + "/api/articles/test-title")
        .then()
        .statusCode(200)
        ...
        .body("article.description", equalTo("New Description"))
