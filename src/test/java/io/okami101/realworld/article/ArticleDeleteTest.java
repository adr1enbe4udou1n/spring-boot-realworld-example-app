...
public class ArticleDeleteTest extends RealworldApplicationTests {
    ...
    public void guest_cannot_delete_article() {
        given().contentType(ContentType.JSON).when().delete(baseUrl + "/api/articles/test-title").then()
                .statusCode(401);
    }

    @Test
    public void cannot_delete_non_existent_article() {
        actingAsJohnUser().contentType(ContentType.JSON).when().delete(baseUrl + "/api/articles/test-title").then()
                .statusCode(404);
    }
    ...
    public void can_delete_article_with_all_comments() {
        ...
        actingAs(john).contentType(ContentType.JSON).when().delete(baseUrl + "/api/articles/test-title").then()
                .statusCode(200);
